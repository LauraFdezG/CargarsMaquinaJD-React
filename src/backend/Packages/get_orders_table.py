...
def get_fiscal_month(date, fiscal_calendar_df: pd.DataFrame):
    ...
    try:
        fiscalYear: datetime.date = fiscal_month["Fiscal Year"][fiscal_month.index[0]]
        fiscal_month: datetime.date = fiscal_month['FiscalMonth'][fiscal_month.index[0]]
        fiscal_month = datetime.datetime(fiscalYear.year, fiscal_month.month, fiscal_month.day)
    except IndexError:
        print(date)
